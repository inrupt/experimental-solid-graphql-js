...
interface IQueryOptions<TData, TVariables>
...
function useSolidQuery<TData, TVariables extends Record<string, any>>(
  ...
  const [state, dispatch] = useReducer<SolidReducer<TData>>(reducer, {
    requesting: false,
    pending: false,
  });
...
export interface QueryProps<TData, TVariables extends Record<string, any>>
  extends IQueryOptions<TData, TVariables> {
  children: (data: TData) => JSX.Element;
  error: (error: ReadonlyArray<GraphQLError>) => JSX.Element;
  fallback: () => JSX.Element;
}

export function Query<TData, TVariables extends Record<string, any>>(
