...
function Spinner() {
  return (
    <svg
      className="animate-spin mr-0.5 h-5 w-5 text-white"
...
function ChevronOrSpinner(props: {
  ...
  return props.loading ? (
    ...
    <ChevronRightIcon
      ...
      className="text-white h-5 w-5 p-1"
    />
  );
}
...
function Provider(props: { name: string; url: string }) {
  ...
  return (
    <Link shallow href={`?provider=${encodeURIComponent(props.url)}`}>
...
function Login() {
  ...
  const [input, setInput] = useState(
    typeof query.provider === "string" && query.provider in providers
      ? strip(query.provider)
      : ""
  );
    ...
    >
      <div className="border-b py-2 mx-5">
        <div className="flex items-center justify-center cursor-pointer">
          ...
          <Link href="/">
            <div className="text-white flex text-3xl p-5">Solid Music</div>
          </Link>
        </div>
      </div>
        ...
        <div className="absolute text-white w-6/12 flex-shrink: 0">
          ...
          <div className="flex-auto mt-4 mb-6 pb-3 border-b bg-grey border-[#2e2e2e]">
            {Object.entries(providers).map(([url, name]) => (
              <Provider key={url} url={url} name={name} />
            ))}
          </div>

          <div className="flex justify-center">or, manually enter the url</div>
          <div className="flex-auto mt-4 bg-grey border-[#2e2e2e]">
            ...
            >
              ...
              {
                <img
                  className={`rounded-full w-${hidden ? "0" : "10"} p-${
                    hidden ? "0" : "1"
                  } h-10 object-cover`}
              ...
              />
              <ChevronOrSpinner
                loading={query.provider === clean(input)}
                onClick={submit}
              />
            ...
            {query.error && (
              <div
                className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 mt-10 rounded relative"
                role="alert"
              >
